...
class CUB_dataset(Dataset):
    ...
    def __init__(self, mode, config_dict: dict,transform=None): 
        ...
        self.n_concepts = 312
        self.crop_size = 299 # Input size for the Inception V3 model
        self.transform = transform

        self.image_dir = os.path.join(config_dict['CUB_dir'],'images') #

        
        #Hard code values for train val test split generated by train_test_val_finder.py
        split = pickle.load(open(os.path.join(r"train_test_val.pkl"), 'rb')) 
        ...
        if config_dict['use_majority_voting']:
            ...
            # Majority voting is based on the training set
            train_ids = split['train']
            concepts, visibility = self.load_concepts(config_dict['CUB_dir']) # Load the concepts and visibility labels
            labels = self.load_labels(config_dict['CUB_dir']) # Load the class labels

            self.concepts, self.concept_mask = self.apply_filter(train_ids,config_dict["min_class_count"],concepts,labels,visibility) # Apply filter to the class attributes
            self.visibility = None # Visibility is not relevant after majority voting
            

            #self.concepts = self.load_concept_paper(mode) # Load concepts from the original CBM paper
        ...
        self.consept_labels_names = pd.read_csv(os.path.join(config_dict['CUB_dir'],"attributes/attributes.txt"), sep=" ", header=None)[1].values
    ...
    def load_concept_paper(self,mode):
        """
        Load majority voting concepts from the original CBM paper
        """

        if mode == 'train':
            with open(os.path.join("Class_attr_data_10/class_attr_data_10/train.pkl"), 'rb') as f:
